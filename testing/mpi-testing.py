...
def train(env_id, num_timesteps, seed, algorithm, model_save_file=None, tensorboard_log=None):

    with tf_util.single_threaded_session():
        rank = MPI.COMM_WORLD.Get_rank()
        if rank == 0:
            logger.configure(folder=os.path.join(tensorboard_log, "legacy"))
        else:
            logger.configure(folder=os.path.join(tensorboard_log, "legacy"))
        ...
        if algorithm == "TRPO":
            model = TRPO(MlpPolicy, env, seed=workerseed, verbose=1, tensorboard_log=tensorboard_log)
        else:
            # Algorithm is PPO
            model = PPO1(MlpPolicy, env, seed=workerseed, verbose=1, tensorboard_log=tensorboard_log)
...
def main():
    ...
    train(args.env, num_timesteps=args.num_timesteps, seed=args.seed, algorithm=args.algorithm,
          model_save_file=model_file, tensorboard_log=log_dir)
